...
class MyGetfundmony extends StatefulWidget {
  const MyGetfundmony({super.key});

  @override
  State<StatefulWidget> createState() => Getfundmony();
}

class Getfundmony extends State<MyGetfundmony> {
  ...
  Future<void> _fetchEvents() async {
   ...
   //   final events = await _db.getEventsBetweenDates(startDate, endDate);
    //  final sum = await _db.getTotalAmountBetweenDates(startDate, endDate);
     final events = await _db.getCategoriesWithCounts(startDate, endDate);
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Align(
            ...
            child: Text("عرض تقرير الجهة الممولة",
                            ...
                            //  isThreeLine: true,
                              ...
                              onTap: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => Myshowbwteendatefundmony(  startDate: date1!, endDate: date2!, fundmony: event['fundingentity'] )
                                  ),
                                );
                              },
                            ),
