...
import 'package:flutter_application_2/showbetweendateAidtype.dart';
import 'package:flutter_application_2/showbwteendatefundmony.dart';
import 'package:flutter_application_2/sqldb.dart';

class MyGetAidtype extends StatefulWidget {
  const MyGetAidtype({super.key});

  @override
  State<StatefulWidget> createState() => Getaidtype();
}

class Getaidtype extends State<MyGetAidtype> {
  ...
  Future<void> _fetchEvents() async {
   ...
   //   final events = await _db.getEventsBetweenDates(startDate, endDate);
    //  final sum = await _db.getTotalAmountBetweenDates(startDate, endDate);
     final events = await _db.getCategoriesWithCounts2(startDate, endDate);
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Align(
            ...
            child: Text("عرض تقرير نوع المساعدة",
                            ...
                            //  isThreeLine: true,
                              ...
                              onTap: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => MyshowbwteendateAidtype(  startDate: date1!, endDate: date2!, fundmony: event['aidtype'] )
                                  ),
                                );
                              },
                            ),
